---
title: "Lab_14T"
author: "Akshay_Gupta_36-600_Lab_07T"
date: "Fall 2023"
output:
  html_document:
    toc: no
...
# FILL ME IN
#head(df,25)
ggplot(df, aes(x=as.factor(x), y=Y))+geom_boxplot(fill="orange")+labs(x = "x (Group)", y="Y (Value)")
```
...
```
```
We reject the null hypothesis for both B and M.
```
...
# FILL ME IN
#head(df,25)
ggplot(df, aes(x=as.factor(x), y=Y))+geom_boxplot(fill="deepskyblue")+labs(x = "x (Group)", y="Y (Value)")
```
...
```
```
We fail to reject the null hypothesis for the Shapiro-Wilk tests and reject the null hypothesis for the Leven test.
```
...
```
```
The estimate mean responses are 2.66656 fpr Group "0", 2.97242 for Group "1", and 3.03095 for Group "2".
```
...
```
```
The hypothesis test statistic value is 0.05 and the p-value is less than that, so we reject the null hypothesis and conclude that at least one of the means is different from the others.
```
...
```
```
Group 0 has a different mean from group 1 and from group 2. The means in groups 1 and 2 are not significantly different.
```
